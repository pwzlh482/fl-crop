"""
serverCropV2 - Crop 联邦学习优化服务端
基于 server_crop.py 重写，完整适配 PFLlib 框架

优化点：
1. Warmup 学习率策略（前 warmup_rounds 轮线性升温，避免初期大梯度）
2. 动态 mu 衰减调度 — 前期高 mu 抑制漂移，后期低 mu 释放个性化
3. 训练过程中追踪最优准确率
4. 服务器端 lr 衰减兼容客户端 CosineAnnealing（不下发覆盖，让客户端自主调度）
5. 适配 PFLlib 完整评估/保存/新客户端微调流程
"""

import time
import torch
from flcore.clients.client_crop_v2 import clientCropV2
from flcore.servers.serverbase import Server


class ServerCropV2(Server):
    ...
    def __init__(self, args, times):
        ...
        self.set_clients(clientCropV2)

        print(f"\nJoin ratio / total clients: {self.join_ratio} / {self.num_clients}")
        print("Finished creating server and clients (CropV2 optimized).")
    ...
    def train(self):
        ...
        if self.num_new_clients > 0:
            ...
            from flcore.clients.client_crop_v2 import clientCropV2
            self.set_new_clients(clientCropV2)
            print(f"\n-------------Fine tuning round-------------")
